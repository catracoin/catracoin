...
const char * const CATRA_CONF_FILENAME = "catra.conf";
const char * const CATRA_PID_FILENAME = "catrad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "catrad -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Catra
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Catra
    // Mac: ~/Library/Application Support/Catra
    // Unix: ~/.catra
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Catra";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Catra";
#else
    // Unix
    return pathRet / ".catra";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CATRA_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", CATRA_PID_FILENAME)));
}
