...
###   catracore.org and catra.org and compares them.
...
WORKINGDIR="/tmp/catra_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://catracore.org"
HOST2="https://catra.org"
BASEDIR="/bin/"
VERSIONPREFIX="catra-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'catra-',
...
if [ $? -ne 0 ]; then
   echo "catra.org failed to provide signature file, but catracore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "catra.org and catracore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Catra Core binary release signing key installed?"
   fi
